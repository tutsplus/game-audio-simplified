...
(function(){
	...
	function main() {
		// Create the sounds.
		// It doesn't matter if the browser can't play these sounds, no errors will be
		// thrown, the sounds will simply not be heard.
		soundA  = new Sound( "sound-01.ogg" );
		soundB  = new Sound( "sound-02.ogg" );
		soundC  = new Sound( "sound-03.ogg", true ); // true = this one loops.

		// Grab the buttons.
		buttonA = document.getElementById( "button-01" );
		buttonB = document.getElementById( "button-02" );
		buttonC = document.getElementById( "button-03" );

		// Turn the master/global volume down a bit because
		// the source sounds are quite loud.
		Sound.setVolume( 0.6 );

		// Set the listener to keep track of file load results.
		Sound.setListener( listener );

		// Load the newly created sounds.
		Sound.load();
	}
	...
	function listener( sound, state ) {
		if( soundA === sound ) {
			buttonA.className += " loaded";
			buttonA.onmousedown = playSound;
			return;
		}

		if( soundB === sound ) {
			buttonB.className += " loaded";
			buttonB.onmousedown = playSound;
			return;
		}

		if( soundC === sound ) {
			buttonC.className += " loaded";
			buttonC.onmousedown = playSound;
			buttonC.onmousemove = updateSound;
			return;
		}
	}
	...
	function playSound( event ) {
		...
		if( buttonA === event.target ) {
			soundA.play();
			return;
		}

		if( buttonB === event.target ) {
			soundB.play();
			soundB.setPan( -1.0 + 2.0 * Math.random() );
			soundB.setVolume( 0.1 + 0.9 * Math.random() );
			return;
		}

		if( buttonC === event.target ) {
			if( playing ) {
				soundC.stop();
			}
			else {
	...
	function updateSound( event ) {
		var n = ( 1 / 300 ) * ( 300 - event.offsetY );
		soundC.setScale( 0.7 + n );
	}
